...
const DashboardView = ({ insights }: { insights: Insights }) => {
  ...
  const getDemandLevelColor: DemandLevelColor = (level) => {
  ...
  const getMarketOutlookInfo = (outlook: string): MarketOutlookInfo => {
    switch (outlook.toLowerCase()) {
      case "positive":
        return { icon: TrendingUpIcon, color: "text-green-500" };
      case "neutral":
        return { icon: LineChartIcon, color: "text-yellow-500" };
      case "negative":
        return { icon: TrendingDownIcon, color: "text-red-500" };
      default:
